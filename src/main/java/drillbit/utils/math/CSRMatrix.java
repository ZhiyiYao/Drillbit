...
import drillbit.utils.math.builders.CSRMatrixBuilder;
...
public final class CSRMatrix extends RowMajorMatrix {
    ...
    public CSRMatrix(@Nonnull int[] rowPointers, @Nonnull int[] columnIndices,
        ...
        super();
        Conditions.checkArgument(rowPointers.length >= 1,
                "rowPointers must be greater than 0: " + rowPointers.length);
        Conditions.checkArgument(columnIndices.length == values.length, "#columnIndices ("
    ...
    public double[] getRow(@Nonnegative final int index) {
        ...
        eachNonZeroInRow(index, new VectorOperation() {
    ...
    public void eachInRow(@Nonnegative final int row, @Nonnull final VectorOperation operation,
        ...
        if (nullOutput) {
            ...
            for (int i = startIn; i < endEx; i++) {
                operation.apply(columnIndices[i], values[i]);
            }
        }
    }
    ...
    public void eachColumnIndexInRow(@Nonnegative final int row,
        ...
        for (int i = startIn; i < endEx; i++) {
            operation.apply(columnIndices[i]);
        }
    }
