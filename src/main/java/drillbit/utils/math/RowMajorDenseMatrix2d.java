...
import drillbit.utils.math.builders.RowMajorDenseMatrixBuilder;
...
public final class RowMajorDenseMatrix2d extends RowMajorMatrix {
    ...
    public double getAndSet(@Nonnegative final int row, @Nonnegative final int col,
        ...
        final double[] rowData = data[row];
        Conditions.checkNotNull(rowData, "row does not exists: " + row);
    ...
    @Override
    public void eachInRow(@Nonnegative final int row, @Nonnull final VectorOperation operation,
        ...
        for (int len = rowData.length; col < len; col++) {
            operation.apply(col, rowData[col]);
        }
        if (nullOutput) {
            for (; col < numColumns; col++) {
                operation.apply(col, 0.d);
            }
        }
    }

    @Override
    public void eachNonZeroInRow(@Nonnegative final int row, @Nonnull final VectorOperation operation) {
        checkRowIndex(row, numRows);
        ...
        for (int col = 0, len = rowData.length; col < len; col++) {
            ...
            if (v != 0.d) {
                operation.apply(col, v);
            }
        }
    }

    @Override
    public void eachColumnIndexInRow(@Nonnegative final int row, @Nonnull final VectorOperation operation) {
        checkRowIndex(row, numRows);
    ...
    @Override
    public void eachInColumn(@Nonnegative final int col, @Nonnull final VectorOperation operation,
                             final boolean nullOutput) {
        ...
        for (int row = 0; row < numRows; row++) {
            final double[] rowData = data[row];
            if (rowData != null && col < rowData.length) {
                operation.apply(row, rowData[col]);
            } else {
                if (nullOutput) {
                    operation.apply(row, 0.d);
                }
            }
        }
    }
    ...
    public void eachNonZeroInColumn(@Nonnegative final int col,
                                    @Nonnull final VectorOperation operation) {
        checkColIndex(col, numColumns);
