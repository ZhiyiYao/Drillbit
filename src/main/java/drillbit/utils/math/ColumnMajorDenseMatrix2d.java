...
import drillbit.utils.math.builders.ColumnMajorDenseMatrixBuilder;
...
public final class ColumnMajorDenseMatrix2d extends ColumnMajorMatrix {
    ...
    public double getAndSet(final int row, final int col, final double value) {
        ...
        final double[] colData = data[col];
        Conditions.checkNotNull(colData, "col does not exists: " + col);
    ...
    @Override
    public void eachInColumn(final int col, @Nonnull final VectorOperation operation,
        ...
        for (int len = colData.length; row < len; row++) {
            operation.apply(row, colData[row]);
        }
        if (nullOutput) {
            for (; row < numRows; row++) {
    ...
    @Override
    public void eachNonZeroInColumn(final int col, @Nonnull final VectorOperation operation) {
        checkColIndex(col, numColumns);
