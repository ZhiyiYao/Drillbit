...
public class svm {
    ...
    // scale function
    public static svm_scale svm_scale_build(ArrayList<ArrayList<FeatureValue>> features) {
        int dims = features.get(0).size();

        double[] feature_max = new double[dims];
        double[] feature_min = new double[dims];
        ...
        for (ArrayList<FeatureValue> record : features) {
            double value;

            for (int index = 0; index < dims; index++) {
                value = record.get(index).getValueAsDouble();
        ...
        return new svm_scale(feature_max, feature_min);
    }

    public static svm_scale readFromBytes(BufferedReader reader) throws IOException {
        int dims = StringParser.parseInt(reader.readLine().trim(), 0);
        ...
        for (int i = 0; i < dims; i++) {
            ...
            min[i] = StringParser.parseDouble(nums[0], 0);
            max[i] = StringParser.parseDouble(nums[1], 0);
        }

        reader.close();

        return new svm_scale(min, max);
    }
    ...
    public static ByteArrayOutputStream svm_save_model_as_flow(svm_model model) throws IOException {
        ByteArrayOutputStream data = new ByteArrayOutputStream();
        DataOutputStream fp = new DataOutputStream(new BufferedOutputStream(data));
