...
//    }
    ...
    public static double[] aryule(@Nonnull final double[] R, @Nonnull final double[] A,
                                  final int order) {
        Conditions.checkArgument(R.length > order,
                "|R| MUST be greater than or equals to " + order + ": " + R.length);
        Conditions.checkArgument(A.length >= order + 1,
    ...
    public static double[] arburg(@Nonnull final double[] X, @Nonnull final double[] A,
                                  final int order) {
        Conditions.checkArgument(X.length > order,
    ...
    public static RealMatrix[][] toeplitz(@Nonnull final RealMatrix[] c, final int dim) {
        Conditions.checkArgument(dim >= 1, "Invalid dimension: " + dim);
        Conditions.checkArgument(c.length >= dim,
    ...
    public static double[] flatten(@Nonnull final RealMatrix[][] grid) {
        Conditions.checkArgument(grid.length >= 1, "The number of rows must be greater than 1");
        Conditions.checkArgument(grid[0].length >= 1,
                "The number of cols must be greater than 1");

        final int rows = grid.length;
        final int cols = grid[0].length;
        RealMatrix grid00 = grid[0][0];
        Conditions.checkNotNull(grid00);
    ...
    public static RealMatrix combinedMatrices(@Nonnull final RealMatrix[][] grid,
        ...
        Conditions.checkArgument(grid[0].length >= 1,
                "The number of cols must be greater than 1");
        Conditions.checkArgument(dimensions > 0, "Dimension should be more than 0: ",
                dimensions);
    ...
    @Nonnull
    public static RealMatrix combinedMatrices(@Nonnull final RealMatrix[] grid) {
        Conditions.checkArgument(grid.length >= 1,
        ...
        for (int row = 0; row < grid.length; row++) {
            RealMatrix cell = grid[row];
            Conditions.checkArgument(cell.getRowDimension() == rowDims,
                    "Mismatch in row dimensions at row ", row);
            Conditions.checkArgument(cell.getColumnDimension() == colDims,
    ...
    public static double power1(@Nonnull final RealMatrix A, @Nonnull final double[] x0,
        ...
        Conditions.checkArgument(A.getColumnDimension() == x0.length,
                "Column size of A and length of x should be same");
        Conditions.checkArgument(A.getRowDimension() == u.length,
                "Row size of A and length of u should be same");
        Conditions.checkArgument(x0.length == v.length, "Length of x and u should be same");
        Conditions.checkArgument(nIter >= 1, "Invalid number of iterations: " + nIter);
    ...
    public static void lanczosTridiagonalization(@Nonnull final RealMatrix C,
        ...
        Conditions.checkArgument(Arrays.deepEquals(C.getData(), C.transpose().getData()),
                "Target matrix C must be a symmetric matrix");
        Conditions.checkArgument(C.getColumnDimension() == a.length,
                "Column size of A and length of a should be same");
        Conditions.checkArgument(T.getRowDimension() == T.getColumnDimension(),
    ...
    public static void tridiagonalQR(@Nonnull final RealMatrix T, @Nonnull final RealMatrix R,
        ...
        Conditions.checkArgument(n == R.getRowDimension() && n == R.getColumnDimension(),
                "T and R must be the same shape");
        Conditions.checkArgument(n == Qt.getRowDimension() && n == Qt.getColumnDimension(),
    ...
    public static void tridiagonalEigen(@Nonnull final RealMatrix T, @Nonnull final int nIter,
        ...
        Conditions.checkArgument(Arrays.deepEquals(T.getData(), T.transpose().getData()),
                "Target matrix T must be a symmetric (tridiagonal) matrix");
        Conditions.checkArgument(eigvecs.getRowDimension() == eigvecs.getColumnDimension(),
                "eigvecs must be a square matrix");
        Conditions.checkArgument(T.getRowDimension() == eigvecs.getRowDimension(),
                "T and eigvecs must be the same shape");
        Conditions.checkArgument(eigvals.length == eigvecs.getRowDimension(),
